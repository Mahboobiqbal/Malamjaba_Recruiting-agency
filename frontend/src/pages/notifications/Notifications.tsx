    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div className="rounded-lg border bg-white dark:bg-slate-900 p-8 text-center shadow-sm dark:shadow-none">
            <Bell className="mx-auto h-12 w-12 text-slate-300" />
            <p className="mt-2 text-slate-500 dark:text-slate-400">No notifications</p>
          </div>
        ) : (
          notifications.map((n: any) => (
            <div key={n.id} className={`rounded-lg border bg-white dark:bg-slate-900 p-4 shadow-sm dark:shadow-none ${!n.is_read ? "border-l-4 border-l-primary" : ""}`}>
              <div className="flex items-start gap-3">
                <div className={`rounded-lg p-2 ${n.type === "warning" ? "bg-amber-50" : n.type === "alert" ? "bg-red-50 dark:bg-red-500/10" : "bg-secondary"}`}>
