...
import { useGetVendorQuery, useUpdateVendorMutation } from "../../services/vendor.service";
...
export default function VendorEdit() {
  ...
  const { data: vendor, isLoading: loading } = useGetVendorQuery(Number(id));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await updateVendor({ id: Number(id), data: { ...form, email: form.email || undefined, contact_person: form.contact_person || undefined, address: form.address || undefined, bank_name: form.bank_name || undefined, account_title: form.account_title || undefined, account_number: form.account_number || undefined, notes: form.notes || undefined } }).unwrap();
  ...
  if (loading) return <div className="text-center py-8 text-slate-500 dark:text-slate-400">Loading...</div>;

  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Edit Vendor</h2>
